...
interface PatientDetailsProps {
  ...
  onAppointmentUpdated: (appointment: Appointment) => void   // ← add
  ...
  async function deletePatient() {
    if (!window.confirm(`Delete ${patient.first_name} ${patient.last_name}? This will also delete their cases and appointments.`)) return
    try {
  ...
  return (
    <section className="page-content">
      ...
      <div className="patient-detail-header">
        <div className="patient-detail-avatar">{patient.first_name[0]}{patient.last_name[0]}</div>
        <div>
          <p className="section-kicker">Patient record</p>
          <h2>{patient.first_name} {patient.last_name}</h2>
          <span>{patient.contact_number || 'No contact number'} · {patient.address || 'No address recorded'}</span>
        </div>
        <div className="patient-detail-actions">
          <button className="text-button" onClick={() => setIsEditing((open) => !open)}>
            {isEditing ? 'Close edit' : 'Edit patient'}
          </button>
          <button className="danger-button" onClick={deletePatient}>Delete patient</button>
        </div>
      </div>
      {isEditing && (
        <PatientEditForm
      ...
      <div className="patient-detail-grid">
        <section className="content-panel">
          <div className="panel-heading">
            <div>
              <p className="section-kicker">Patient cases</p>
              <h3>{patient.patient_cases.length} case{patient.patient_cases.length === 1 ? '' : 's'}</h3>
            </div>
            <button className="primary-button" onClick={() => setIsFormOpen((open) => !open)}>
              {isFormOpen ? 'Close form' : 'Add case'}
            </button>
          </div>
          {isFormOpen && (
            <PatientCaseForm
              ...
              onCreated={(patientCase) => {
                // Update the parent so this component re-renders with the new case.
                onUpdated({
                  ...patient,
                  patient_cases: [...patient.patient_cases, patientCase],
                })
                setIsFormOpen(false)
              }}
            />
          ...
          {patient.patient_cases.length === 0 ? (
            ...
            patient.patient_cases.map((patientCase) => (
              <PatientCaseRow
                key={patientCase.id}
                ...
                onUpdated={(updated) => {
                  onUpdated({
                    ...patient,
                    patient_cases: patient.patient_cases.map((item) =>
                ...
                onDeleted={(id) => {
                  onUpdated({
                    ...patient,
                    patient_cases: patient.patient_cases.filter((item) => item.id !== id),
                  })
                }}
              />
            ))
        ...
        <section className="content-panel patient-info-panel">
          <p className="section-kicker">Patient information</p>
          <h3>Notes</h3>
          <p className="detail-notes">{patient.notes || 'No notes recorded.'}</p>
          <p className="section-kicker">Date of birth</p>
          <p className="detail-notes">{patient.date_of_birth || 'Not provided'}</p>
        </section>
      </div>
      <section className="content-panel patient-appointments-panel">
        <div className="panel-heading">
          <div>
            <p className="section-kicker">Patient appointments</p>
            <h3>{appointments.length} appointment{appointments.length === 1 ? '' : 's'}</h3>
          </div>
        </div>
        {appointments.length === 0 ? (
          <p className="empty-state">No appointments for this patient.</p>
        ) : (
...
function PatientCaseForm({ patientId, caseTypes, onCreated }: { patientId: number; caseTypes: CaseType[]; onCreated: (patientCase: PatientCase) => void }) {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) { event.preventDefault(); setError(null); setIsSaving(true); try { onCreated(await api.createPatientCase({ patient_id: patientId, case_type_id: Number(caseTypeId), details: details || null })) } catch { setError('Unable to add this case. Select a valid case type and try again.') } finally { setIsSaving(false) } }
    return (
    <form className="patient-case-form" onSubmit={submit}>
      {error && <div className="api-alert" role="alert">{error}</div>}
      ...
      <div className="form-grid">
        <label>
          ...
          >
            <option value="">Select configured case type</option>
            {caseTypes.map((ct) => (
        ...
        <label className="form-notes">
          ...
          <textarea
            ...
            placeholder="Add patient-specific case details"
          />
        </label>
      </div>
  ...
  function PatientCaseRow({
    ...
    patientId: number;
    caseTypes: CaseType[];
    onUpdated: (patientCase: PatientCase) => void;
    ...
    async function save() {
      try {
        onUpdated(
          await api.updatePatientCase(patientCase.id, {
    ...
    // ---- EDIT STATE: renders a form, not the read-only row ----
    if (isEditing) {
      return (
        <div className="detail-case-edit">
          {error && <div className="api-alert" role="alert">{error}</div>}
    ...
    // ---- DISPLAY STATE ----
    const caseName = patientCase.case_type?.name || 'Unnamed case';
    const initials = caseName.slice(0, 2).toUpperCase();

    return (
      <>
        <article className="detail-case">
          <div className="case-type-icon">{initials}</div>

          <div className="detail-case-body">
            <strong>{caseName}</strong>
            <span>{patientCase.details || 'No case details recorded'}</span>
            {patientCase.images && patientCase.images.length > 0 && (
              <small>
                {patientCase.images.length} image
                {patientCase.images.length === 1 ? '' : 's'}
              </small>
            )}
          </div>

          <div className="detail-case-actions">
            <button
              type="button"
              className="text-button"
        ...
        <TaskList tasks={patientCase.tasks || []} />
      </>
    );
  }
